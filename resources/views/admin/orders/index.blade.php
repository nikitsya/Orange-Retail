            ...
            <section class="section-panel stack">
                ...
                <form class="form-grid-3" method="GET" action="{{ route('admin.orders.index') }}">
                    <label class="field-label">
                        Search
                        <input class="field" type="search" name="search" value="{{ $search }}" placeholder="Order number, customer, email">
                    </label>
                    ...
                    <label class="field-label">
                        ...
                        <select class="field-select" name="status">
                            ...
                            @foreach ($statuses as $orderStatus)
                                <option value="{{ $orderStatus }}" @selected($orderStatus === $status)>{{ ucfirst($orderStatus) }}</option>
                            @endforeach
                        </select>
                    </label>
                        ...
                        <article class="summary-panel order-card">
                            <div class="order-card-head">
                                <div>
                                    ...
                                    <p>{{ $order->customer_name }} | {{ $order->customer_email }} | {{ $order->placed_at?->format('d M Y H:i') }}</p>
                                </div>
                            ...
                            <div class="tile-actions order-card-actions">
                                <form class="inline-form" method="POST" action="{{ route('admin.orders.update', $order) }}">
                                    ...
                                    <select class="field-select" name="status">
                                        @foreach ($statuses as $orderStatus)
                                            <option value="{{ $orderStatus }}" @selected($orderStatus === $order->status)>{{ ucfirst($orderStatus) }}</option>
                                        @endforeach
                                    </select>
