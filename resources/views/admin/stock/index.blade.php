        ...
        <main class="page-shell page-main stock-layout">
            <section class="section-panel stack">
                ...
                <div>
                    ...
                    <p class="muted-copy">Review live stock, last restock dates, next expected deliveries, and record manual warehouse updates.</p>
                </div>
                ...
                <form class="form-grid-3" method="GET" action="{{ route('admin.stock.index') }}">
                    <label class="field-label">
                        Search
                        <input class="field" type="search" name="search" value="{{ $search }}" placeholder="Product, SKU, brand">
                    </label>
                    ...
                    <label class="field-label">
                        ...
                        <select class="field-select" name="category">
                            ...
                            @foreach ($categories as $catalogCategory)
                                <option value="{{ $catalogCategory }}" @selected($catalogCategory === $category)>{{ $catalogCategory }}</option>
                            @endforeach
                        </select>
                    </label>
                    ...
                    @foreach ($products as $product)
                        <article class="summary-panel stock-card">
                            <div class="order-card-head">
                                ...
                                <div class="stock-pill {{ $product->stock === 0 ? 'is-danger' : ($product->stock <= 5 ? 'is-warning' : 'is-ok') }}">
                            ...
                            <form class="stack" method="POST" action="{{ route('admin.stock.update', $product) }}">
                                ...
                                <div class="form-grid-3">
                                    <label class="field-label">
                                        Current stock
                                        <input class="field" type="number" min="0" name="stock" value="{{ old('stock', $product->stock) }}" required>
                                    </label>

                                    <label class="field-label">
                                        Last restocked at
                                        <input class="field" type="datetime-local" name="last_restocked_at" value="{{ old('last_restocked_at', $product->last_restocked_at?->format('Y-m-d\\TH:i')) }}">
                                    </label>

                                    <label class="field-label">
                                        Next delivery due
                                        <input class="field" type="datetime-local" name="next_delivery_due_at" value="{{ old('next_delivery_due_at', $product->next_delivery_due_at?->format('Y-m-d\\TH:i')) }}">
                                    </label>
                                </div>

                                <label class="field-label">
                                    Stock note
                                    <textarea class="field-area field-area-compact" name="stock_note">{{ old('stock_note') }}</textarea>
                                </label>
